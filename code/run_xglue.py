...
def convert_examples_to_features_clone_eval(js, tokenizer, path_dict, args):
    ...
    for i in [1, 2]:
        ...
        source_embeds, cfg_allpath = path_dict[js[f'idx{i}']]

        codes_paths.append(source_embeds)
    codes_paths = torch.tensor(codes_paths, dtype=torch.float32)
    return CloneFeatures(codes_paths, label=js['label'])
...
class CloneFeatures(object):
    def __init__(self,
        ...
        self.input_path_embeds = input_path_embeds
        self.label = label
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, i):
        return (self.examples[i].input_path_embeds,
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        model.to(args.device)
        results=evaluate(args, model, tokenizer,pool=pool)
